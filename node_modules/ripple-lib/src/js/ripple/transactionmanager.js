...
TransactionManager.prototype._transactionReceived = function(tx) {
  ...
  if (!(submission instanceof Transaction)) {
    // The received transaction does not correlate to one submitteo
...
TransactionManager.prototype._prepareHex = function(tx) {
  var submitRequest = this._remote.requestSubmit();
...
TransactionManager.prototype._request = function(tx) {
  ...
  // Override here to get the hex representation of the transaction

  var hex = this._prepareHex(tx);
...
TransactionManager.prototype.submit = function(tx, callback) {
  console.log('manager submit!!', callback)
  var self = this;
  var remote = this._remote;

  if (typeof this._nextSequence !== 'number') {
    // If sequence number is not yet known, defer until it is.
    console.log('no _nextSequence')
  ...
  console.log('_pending.push(tx)')
  this._pending.push(tx);

  console.log('about to _request')
  var hex = this._request(tx)
  console.log('HEX _request', hex)
  callback(hex);
};
